...
def AdjustLight(curZone):
	...
	maxStartingValue = 155
	subtractionModifier = 30
	finalValueToSet = 0
	...
	if (receivedData.LightSensorValue > 0 and receivedData.LightSensorValue <= 50):
		finalValueToSet = maxStartingValue * userControlModifier 
	
	elif (receivedData.LightSensorValue > 51 and receivedData.LightSensorValue <= 100):
		finalValueToSet = (maxStartingValue * userControlModifier) - (1* subtractionModifier)
	
	elif (receivedData.LightSensorValue > 101 and receivedData.LightSensorValue <= 150):
		finalValueToSet = (maxStartingValue * userControlModifier) - (2* subtractionModifier)
	
	elif (receivedData.LightSensorValue > 151 and receivedData.LightSensorValue <= 200):
		finalValueToSet = (maxStartingValue * userControlModifier) - (3* subtractionModifier)
	
	elif (receivedData.LightSensorValue > 201 and receivedData.LightSensorValue <= 250):
		finalValueToSet = (maxStartingValue * userControlModifier) - (4* subtractionModifier)
	
	elif (receivedData.LightSensorValue > 250):
		finalValueToSet = 0
	...
	if (energySavingModeOn == 1):
		...
		finalValueToSet = finalValueToSet * smartGridModifier

	
	#finally set the values 
	setLightBrightness(curZone,finalValueToSet)
	updateBrirghtnessSetting(curZone.ZoneId,finalValueToSet)
...
def setLightBrightness(curZone,value):
	...
	for bulb in mybulbList:
		bridge.set_light(bulb,'bri', value)	
		time.sleep (0.2)	
